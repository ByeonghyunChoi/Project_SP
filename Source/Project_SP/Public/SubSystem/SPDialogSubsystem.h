...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnDialogRowUpdated, const FDialogRow&, DialogRowData);
// 대화가 끝났을때를 알려주기 위해
...
public:
	...
	void StartDialog(UDataTable* DialogTable, APlayerController* PC, TSubclassOf<class UUserWidget> DialogWidgetClass);
	// 대화 바로 띄우기 함수
	UFUNCTION(BlueprintCallable, Category = "Dialog")
	void PlayNextDialog();
	// 3. 대화 종료 함수
...
private:
	...
	TArray<FDialogRow> CurrentDialogRows; // 검색으로 띄우는 대신 배열에 담아놓고 +1씩으로 편하기 읽어나가기 위함
